...
export default function AdminCreateFeature() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.ok) { alert("Feature berhasil ditambahkan!"); router.push("/admin/dashboard/features"); }
  };

  return (
    <div className="p-8">
      <h1 className="text-2xl font-bold mb-6">Tambah Feature Baru</h1>
      <form onSubmit={handleSubmit} className="space-y-4 max-w-2xl">
        ...
        <div><label className="block mb-2">Icon (emoji)</label><input type="text" className="w-full border p-2 rounded" value={formData.icon} onChange={(e) => setFormData({...formData, icon: e.target.value})} /></div>
